...
RF-A* iteratively constructs partial paths: at each step it identifies the next refueling
...
\begin{enumerate}
    ...
    \item \textbf{Tighter price estimate in favorable regimes.}
          ...
          $p_{\min}^{\mathrm{reach}}$ is often above the global minimum, so the fuel-need
          reduction $\max(d_{\mathrm{geo}}/\eta - f,\;0)$ typically dominates and
          $h_{\mathrm{PF}}$ behaves as a tighter bound in practice. A universal pointwise
          dominance over $h_{\mathrm{RF}}$ is \emph{not} claimed; see
