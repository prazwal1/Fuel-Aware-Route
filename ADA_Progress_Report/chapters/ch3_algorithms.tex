...
This chapter presents the modelling assumptions, data structures, and algorithmic methods used to solve the fuel-aware route optimisation problem. The objective is to determine a route from source~$s$ to destination~$t$ that minimises total refuelling cost while respecting vehicle fuel-capacity constraints. Six algorithmic strategies are considered: State-Expanded Dijkstra, a Greedy baseline, Dynamic Programming (DP), Standard A*, Refuel A* (RF-A*), and the proposed Partial-Fill A* (PF-A*).
...
Let $d(u,v)$ denote the travel distance from node~$u$ to node~$v$, let $\eta$ denote vehicle fuel efficiency in km/L, and let $p(u)$ denote the fuel price at station~$u$. The fuel cost incurred when departing from~$u$ and travelling to~$v$ is defined as
\[
...
A six-node network is used throughout this chapter to illustrate algorithm behaviour. Nodes A--E are fuel stations, and T is the destination. The vehicle begins at node~A with an initial fuel level of $f_0=28$~L and maximum capacity $C=40$~L. Fuel efficiency is fixed at $\eta=10$~km/L.
...
State-Expanded Dijkstra applies Dijkstra's shortest-path method to the state-expanded graph~$G^{*}$. Since each state encodes both location and remaining fuel, the algorithm jointly optimises routing and refuelling.
...
In this example, the algorithm correctly postpones refuelling until station~E, where the fuel price is lowest. The resulting minimum cost is 216~THB.
...
O(EF), & \text{optimised implementation},
\end{cases}
\qquad
\text{Space}=O(VF),
\]
...
RF-A* is a refuelling-oriented variant of A* that applies a fuel-agnostic lower bound,
\[
...
RF-A* follows the same route as the optimal solution but makes poor refuelling decisions. Because $h_{\mathrm{RF}}(v)$ is independent of fuel level, states $(A,28)$ and $(A,40)$ receive the same heuristic treatment, causing the algorithm to over-purchase at the origin. The total cost is therefore 864~THB.
...
A central issue in this research is the effect of allowing partial refuelling decisions.

\paragraph{Full-tank-only model.}
If the driver must always fill the tank completely, fuel level no longer behaves as a decision variable. The problem reduces to route selection over the original graph, with each refuelling event fixed at capacity. This model is computationally simpler but may purchase more fuel than necessary.
...
Variable purchase & DP & $O(VF)$ & $O(EF)$ in optimised form & Yes \\
...
Synthetic networks are generated to evaluate algorithm behaviour under controlled changes in station density, graph size, fuel capacity, initial fuel, and price variance. The procedure is as follows:
\begin{enumerate}
